...
const scenarioCollectionCache = new Map()
const SCENARIO_CACHE_TTL_MS = 15000
...
async function readCollection(collectionName) {
  const cached = scenarioCollectionCache.get(collectionName)
  if (cached && Date.now() - cached.timestamp < SCENARIO_CACHE_TTL_MS) {
    return cached.data
  }
  ...
  try {
    ...
    const result = await db.collection(collectionName).find({}).project({ _id: 0 }).toArray()
  ...
  if (!rows) {
    rows = readLocal(collectionName)
  }

  scenarioCollectionCache.set(collectionName, { data: rows, timestamp: Date.now() })
  return rows
}
